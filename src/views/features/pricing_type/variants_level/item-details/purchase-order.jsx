...
class PurchaseOrderComponent extends BaseComponent {
    ...
    addItemToCart(e) {
        ...
        this.props.getUserCarts(options, function(cartList) {
            ...
            self.props.addOrEditCart(cartItemId, quantityToPass, 0, item.ID, true, false,
    ...
    contactSeller() {
        ...
        let item = parentItem.HasChildItems ? this.getItem() : self.props.itemDetails;
                    ...
                    return Object.values(VariantGroups).map(vg => 
                        <span className="full-width" key={vg[0].GroupName}>
                            ...
                            <span className="idcrtl-right full-width relation">
                                <select className="full-width required" data-group-id={vg[0].GroupID} defaultValue=''>
                                    <option value={''}></option>
                                    {
                                        vg.map(variant => 
                                            <option 
    ...
    handleAddToCartSuccess() {
        ...
        $('.h-cart .h-dd-menu.add-cart.fadeout > .h-cart-mid > ul > .cart-item > .item-img > img').attr('src', (addedItem.Media ? addedItem.Media && addedItem.Media[0].MediaUrl : '') || Media? Media[0].MediaUrl:'');
        ...
        $('.h-cart .h-dd-menu.add-cart.fadeout > .h-cart-mid > ul > .cart-item > .item-info > .item-price > .value').text(Numeral(cartSubTotal).format(process.env.MONEY_FORMAT));
        ...
        return (
            <div className="idcr-top pull-left w-100">
                ...
                <div className="idcrt-order-btn pull-left w-100">
                    <div className={"btn-group btn-cart " + isSpotPurchase} id="itemAddCart" onClick={(e) => this.addItemToCart(e)}>Add to Cart</div>
                    <div className={"btn-group contact-btn " + isNegotiate} id="negotiate" onClick={() => this.contactSeller()}>Negotiate</div>
                </div>
            </div>
        );
    }
