...
class CalendarView extends BaseComponent {
	                	...
	                	if (blockedTimeRange) {
		                	for (let i = 0; i < blockedTimeRange.length; i+=1) {
		                		if (blockedTimeRange[i]) {
		                    		const startStr = Moment().set(blockedTimeRange[i].start).format('HH:mm');
									const endStr = blockedTimeRange[i].end.h == 24 ? '23:59' : Moment().set(blockedTimeRange[i].end).format('HH:mm');
									//if restdays should not put unavailables
		           //         		restDays.push({
		           //                 	title: "Unavailable",
		    			    //			allDay: false,
						    			//textColor :'#ffffff',
    ...
    getBookingDatesEvents() {
    	const self = this;
    	if (this.props.bookings && this.props.bookings.length > 0 && this.props.itemDetails) {
    		let helper = {};
	...
	render() {
		return (
			<div className="idclt-custom-field full-width inline-available-dates">
