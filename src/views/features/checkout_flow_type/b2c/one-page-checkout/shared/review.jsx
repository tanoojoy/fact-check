...
class ReviewComponent extends BaseComponent {
    ...
    renderCartItem(cartItem, i) {
       ...
       // const cartSubTotal = parseFloat(cartItem.SubTotal) - parseFloat(cartItem.DiscountAmount || 0);

        //ARC9207
        const cartSubTotal = parseFloat(ItemDetail.Price * cartItem.Quantity) - parseFloat(cartItem.DiscountAmount || 0);
        return (
            <tr key={cartItem.ID + i}>
                <td data-th="Item Description">
                    {this.renderCartItemDescription(ItemDetail, i)}
                </td>
                <td data-th="Quantity">{this.formatAmountWithCommaSeparator(cartItem.Quantity)}</td>
