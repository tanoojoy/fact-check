...
class TransactionSummaryComponent extends BaseComponent {
    ...
    renderRecievingNotes() {
        let self = this;
        if (this.props.detail && this.props.detail.ReceivingNotes && this.props.detail.ReceivingNotes.length > 0) {
            let rNotes = this.props.detail.ReceivingNotes.map((d) =>
                '<div>' + d.ReceivingNoteNo + '</div>'
            )
            return <div dangerouslySetInnerHTML={{ __html: rNotes.join('') }} />
        }
        else {
            return '-'
        }
    ...
    render() {
        ...
        if (detail.RequisitionDetail) {
            requisitionOrderNo = detail.RequisitionDetail.RequisitionOrderNo;
        }
        const quoteRoute = quoteID ? `/quotation/detail?id=${quoteID}` : '#';
        const requisitionRoute = detail.RequisitionDetail.ID ? `/requisition/detail?id=${detail.RequisitionDetail.ID}` : '#';
                            ...
                            <table className="canon-table">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td data-th="Receiving Notes. :"><a href="#"><span className="highlight-text">{self.renderRecievingNotes()}</span></a></td>
                                    </tr>
                                    <tr>
                                        <th>Invoice No. :</th>
