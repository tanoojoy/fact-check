...
class AddressBoxComponent extends BaseComponent {
    render() {
        const { address, setDeliveryAddress, showDeleteAddress, showFirst, showSecond } = this.props;
        ...
        return (
            <div key={address.ID} onClick={(e) => setDeliveryAddress(address.ID)} className="pdcb-address-box  parent-r-b onboarder-address" style={{ 'height': '210px' }}>
                ...
                <span className="icon-delete openModalRemove"><i className="fa fa-trash" onClick={(e) => showDeleteAddress(address.ID)}></i></span>
            </div>
        );
    }
}
