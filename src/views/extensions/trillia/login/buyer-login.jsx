...
class BuyerLoginComponent extends BaseComponent {
    componentDidMount() {
        let self = this;
        ...
        return (
            <div key="google" className="btn-google">
                <a href={this.props.googleLoginUrl}>
                    <i className="fa fa-google"></i><span>Google Login</span>
                </a>
            </div>
        );
    }
        ...
        return (
            <div key="facebook" className="btn-fb">
                <a href={this.props.facebookLoginUrl}>
                    <i className="fa fa-facebook-square"></i><span>Facebook Login</span>
                </a>
            </div>
        );
    }
    ...
    renderCustomLogin() {
        ...
        let actionLink = '/accounts/buyer/sign-in';
        if (this.props.chatChannelId && this.props.chatChannelId.length > 1) {
            actionLink = '/accounts/buyer/sign-in?chatChannelId=' + this.props.chatChannelId;
        }

        return (
            <form key="custom-login" id="frmlogin" action={actionLink} className="custom-login" method="post" autoComplete="off">
                <div className="lbb-input">
                    <input type="text" className="input-text" placeholder="Email/Username" id="username" name="username" onKeyPress={(e) => this.handleKeyPress(e)} />
                    <input type="password" className="input-text" placeholder="Password" id="password" name="password" onKeyPress={(e) => this.handleKeyPress(e)} autoComplete="off" />
                </div>
    ...
    render() {
        ...
        let hrefLink = '/';
        if (this.props.chatChannelId && this.props.chatChannelId.length > 1) {
            hrefLink = '/accounts/sign-in?chatChannelId=' + this.props.chatChannelId;
        }
        return (
            <React.Fragment>
                    ...
                    <div className="login-box">
                        ...
                        <div className="lb-body full-width"><span className="b-gray">Sign In</span>
                            ...
                            <span className="lbb-text">
                                <a href="/accounts/buyer/forgot-password">Forgotten your password?</a>
                            </span>
...
const BuyerLoginHome = ReactRedux.connect(
    mapStateToProps,
    mapDispatchToProps
)(BuyerLoginComponent)
